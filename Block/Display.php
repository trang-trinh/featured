<?php
namespace Magepow\Featured\Block;
...
class Display extends \Magento\Catalog\Block\Product\ListProduct
{
    ...
    public function __construct(
        ...
        \Magepow\Featured\Helper\Data $helper,
        ...
        $this->pageConfig->getTitle()->set(__($this
            ->_helper
            ->getConfigValue(
                'featured_settings/featured_products/title'
            )));
    }
    ...
    protected function getProducts()
    {
        ...
        $limit = $this->_helper
            ->getConfigValue(
                'featured_settings/featured_products/limit'
            );
        if ($limit == "" || $limit == 0) {
            $limit = 10;
        }
        ...
        $collection = $this->_collection
            ...
            ->addAttributeToFilter('is_saleable', array_map('intval', str_split(1)), 'left')
            ->addAttributeToFilter('featured', 1, 'left');

        $pager = $this->getLayout()
            ->createBlock(
                'Magento\Theme\Block\Html\Pager',
                'featured.grid.record.pager'
            )
            ->setLimit($limit)
